...
template <typename T> int cmp_nanequal(T* in1, T* in2) {
  T const *p1, *p2;
  for(p1 = in1, p2 = in2; (*p1 == *p2) || (isnan(*p1) && isnan(*p2)); ++p1, ++p2)
    continue;
  return p1 - in1;
}
template <typename T> int cmp_missing_nanequal(T* in1, T* in2, T m1, T m2) {
  T const *p1, *p2;
  for(p1 = in1, p2 = in2; (*p1 == *p2) || ((*p1 == m1) && (*p2 == m2)) || (isnan(*p1) && isnan(*p2)); ++p1, ++p2)
    continue;
  return p1 - in1;
}
...
template <typename T> int cmp_var ( int ncid1, int ncid2, nccmpopts* opts, int rec, size_t *odomax,
    ...
    char idxstr[256];
    std::string message ("DIFFER : VARIABLE : %s : POSITION : %s : VALUES : %s <> %s\n");
    ...
    do {
      ...
      while (diff < cmplen && (opts->maxdiff == 0 || diff_cnt < opts->maxdiff) ) {
        //  printf("RJH: maxdiff=%ld cnt=%ld\n",opts->maxdiff,diff_cnt);
        ...
        printf( message.c_str(), v1->name, idxstr, value1str, value2str);
...
template <typename T> int cmp_vartol (int ncid1, int ncid2, nccmpopts* opts, int rec, size_t *odomax,
    ...
    T *P1, *P2;
        ...
        int  i,status, cmplen, do_missing=0, diffstatus = EXIT_SUCCESS;
        long diff_cnt = 0;
        char idxstr[256];
        std::string message ("DIFFER : VARIABLE : %s : POSITION : %s : VALUES : %s <> %s : PERCENT : %g\n");
        char value1str[32], value2str[32];
        char printHex = strchr(opts->precision, 'X') || strchr(opts->precision, 'x');
        double absdelta;
    ...
    do {
      ...
      for(i=0; i < nitems && (opts->maxdiff == 0 || diff_cnt < opts->maxdiff) ; ++i)
      {
        if (do_missing) {
        ...
        if (opts->abstolerance ? (absdelta > opts->tolerance) : (double)absdelta*100./(fabs((double)P1[i]) > fabs((double)P2[i]) ? fabs((double)P1[i]) : fabs((double)P2[i])) > opts->tolerance)  \
        {
          ...
          fprintf(stderr, message.c_str(), v1->name, idxstr, value1str, value2str, (double)absdelta*100./(fabs((double)P1[i]) > fabs((double)P2[i]) ? fabs((double)P1[i]) : fabs((double)P2[i])) );
          diff_cnt++;
          if (! opts->force)
          {
            goto break_;
...
int cmpattval(int nc1, int nc2, int varid1, int varid2, char* name, int len, nc_type type)
{
  ...
  case NC_UBYTE:
          ...
          if (status != NC_NOERR)
          {
                  XFREE(uuc1); XFREE(uuc2); return NC_EINVAL;
          }
          ...
          for(i = 0; i < len; ++i)
                    {
                            if (uuc1[i] != uuc2[i])
                            {
                                    XFREE(uuc1); XFREE(uuc2); return EXIT_DIFFER;
                            }
                    }
         XFREE(uuc1);
         XFREE(uuc2);
  ...
  case NC_USHORT:
          ...
          if (status != NC_NOERR)
          {
                  XFREE(us1); XFREE(us2); return NC_EINVAL;
          }
          ...
          for(i = 0; i < len; ++i)
          {
                  if (us1[i] != us2[i])
                  {
                          XFREE(us1); XFREE(us2); return EXIT_DIFFER;
                  }
          }
         XFREE(us1);
         XFREE(us2);
...
void broadcast_missing(nc_type var_type, nc_type att_type, missing_struct *values) {
  ...
  switch(var_type) {
    ...
    case NC_UBYTE: BROADCAST_MISSING(ub); break;
    case NC_USHORT: BROADCAST_MISSING(us); break;
...
nccmpglobalatts(nccmpopts* opts, int ncid1, int ncid2)
{
  ...
  const std::string hstry("history");
...
int cmpvar(char* name, int rec, nccmpopts* opts, int ncid1, int ncid2)
{
  ...
  case NC_INT:
      strcpy(opts->tprintf, "%d");
      diffstatus =  cmp_var<int>(ncid1, ncid2, opts, rec, odomax, nitems, count, start, v1, v2, v1->missing.i, v2->missing.i);
    break;
  
  case NC_FLOAT:
      strcpy(opts->tprintf, opts->precision);
      if (opts->notolerance) {
          diffstatus =  cmp_var<float>(ncid1, ncid2, opts, rec, odomax, nitems, count, start, v1, v2, v1->missing.f, v2->missing.f);
      }else{
          if (opts->verbose)
...
nc_inq_grps(ncid2, &numgrps2, NULL);
  if (numgrps2 > 0){
      groups2 = (GROUP_NODE *)malloc(sizeof(GROUP_NODE)*numgrps2);
