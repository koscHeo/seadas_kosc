                            ...
                            EPR_SRaster* bm_raster)
{
    ...
    bm_buffer = (epr_uchar*) bm_raster->buffer;
    if (bm_buffer == NULL) {
         epr_set_err(e_err_out_of_memory,
    ...
    for (y = 0; y < bm_raster->raster_height; y++) {
        for (x = 0; x < bm_raster->raster_width; x++) {
            bm_buffer[pos] = (epr_uchar) epr_eval_bm_term(context, term, x, y);
            pos++;
            errcode = epr_get_last_err_code();
            if (errcode != 0) {
                break;
    ...
    case BMT_REF:
		{
            ...
            epr_uint flag_mask = term->op.ref.flag_mask;
...
void epr_resolve_bm_ref(EPR_SBmEvalContext* context, EPR_SBmTerm* term) {
    ...
    epr_uint flag_band_index = (epr_uint) -1;
    epr_uint band_index = 0;
    epr_uint num_bands = context->flag_band_ids->length;
    EPR_SRaster* flag_raster = NULL;
    epr_uint flag_mask = 0;
	epr_uint flag_computed = 0;
...
EPR_SBmTerm* epr_parse_bm_expr_str(const char* bm_expr) {
    ...
    parse_info.pushed_back = EPR_FALSE;
    parse_info.token = NULL;
    parse_info.err_message = NULL;

    term = epr_parse_bm_expr(&parse_info, EPR_FALSE);
...
EPR_SPtrArray* epr_create_flag_coding(EPR_SProductId* product_id, const char* flag_coding_name)
{
	...
	for (i = 0; i < num_descr; i++) {
        ...
        /* 2: dataset_name */
        /* flag_def->bit_index = (epr_uint)fc_tables[fct_index].descriptors[i].bit_index; */
		flag_def->bit_mask = 0;
...
EPR_SBmTerm* epr_parse_bm_OR_expr(EPR_SParseInfo* parse_info, epr_boolean term_required) {
    ...
    while (!epr_is_bm_expr_error(parse_info)) {
        ...
        if (epr_is_bm_OR_keyword(parse_info) || epr_is_bm_OR_operator(parse_info)) {
            EPR_SBmTerm* term2 = epr_parse_bm_OR_expr(parse_info, EPR_TRUE);
...
EPR_SBmTerm* epr_parse_bm_AND_expr(EPR_SParseInfo* parse_info, epr_boolean term_required) {
    ...
    while (!epr_is_bm_expr_error(parse_info)) {
        ...
        if (epr_is_bm_AND_keyword(parse_info) || epr_is_bm_AND_operator(parse_info)) {
            EPR_SBmTerm* term2 = epr_parse_bm_AND_expr(parse_info, EPR_TRUE);
...
EPR_SBmTerm* epr_parse_bm_unary_expr(EPR_SParseInfo* parse_info, epr_boolean term_required)  {
    ...
    if (epr_is_bm_NOT_keyword(parse_info) || epr_is_bm_NOT_operator(parse_info)) {
        term = epr_parse_bm_unary_expr(parse_info, EPR_TRUE);
...
EPR_SBmTerm* epr_parse_bm_primary_expr(EPR_SParseInfo* parse_info, epr_boolean term_required) {
    ...
    if (epr_get_token_char(parse_info) == '(') {
        term = epr_parse_bm_expr(parse_info, EPR_TRUE);
        epr_next_bm_expr_token(parse_info);
        if (epr_get_token_char(parse_info) != ')') {
...
void epr_push_back_bm_expr_token(EPR_SParseInfo* parse_info) {
    parse_info->pushed_back = EPR_TRUE;
}
...
epr_uint epr_get_pixel_as_uint(const EPR_SRaster* raster, int x, int y)
{
    ...
    case (e_tid_uchar) :
          return (epr_uint) ((epr_uchar*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_char)  :
          return (epr_uint) ((char*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_ushort):
          return (epr_uint) ((epr_ushort*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_short) :
          return (epr_uint) ((short*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_uint):
          return (epr_uint) ((epr_uint*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_int) :
          return (epr_uint) ((int*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_float) :
          return (epr_uint) ((float*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_double) :
          return (epr_uint) ((double*)raster->buffer)[y * raster->raster_width + x];
    default:
          return 0;
    }
}
...
int epr_get_pixel_as_int(const EPR_SRaster* raster, int x, int y)
{
    ...
    switch (raster->data_type) {
    case (e_tid_uchar) :
          return (int) ((epr_uchar*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_char)  :
          return (int) ((char*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_ushort):
          return (int) ((epr_ushort*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_short) :
          return (int) ((short*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_uint):
          return (int) ((epr_uint*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_int) :
...
float epr_get_pixel_as_float(const EPR_SRaster* raster, int x, int y)
{
    ...
    switch (raster->data_type) {
    case (e_tid_uchar) :
          return (float) ((epr_uchar*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_char)  :
          return (float) ((char*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_ushort):
          return (float) ((epr_ushort*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_short) :
          return (float) ((short*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_uint):
          return (float) ((epr_uint*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_int) :
...
double epr_get_pixel_as_double(const EPR_SRaster* raster, int x, int y)
{
    ...
    switch (raster->data_type) {
    case (e_tid_uchar) :
          return (double) ((epr_uchar*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_char)  :
          return (double) ((char*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_ushort):
          return (double) ((epr_ushort*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_short) :
          return (double) ((short*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_uint):
          return (double) ((epr_uint*)raster->buffer)[y * raster->raster_width + x];
    case (e_tid_int) :
