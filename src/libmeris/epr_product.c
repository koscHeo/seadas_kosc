...
EPR_SProductId* epr_open_product(const char* product_file_path) {
    ...
    if (fread(product_id->id_string,
              ...
              product_id->istream) != (epr_uint) EPR_PRODUCT_ID_STRLEN) {
        epr_set_err(e_err_file_access_denied,
    ...
    /* Get file size */
    product_id->tot_size = (epr_uint) ftell(product_id->istream);
    if (product_id->tot_size == (epr_uint) -1) {
        epr_set_err(e_err_file_access_denied,
    ...
    epr_set_dyn_dddb_params(product_id);
