...
*/
    ...
    else if (field->info->data_type_id == e_tid_time)
    {
        ...
        for (i = 0; i < field->info->num_elems; i++)
        {
            ...
            {
            case e_tid_uchar:
                fprintf(ostream, "%u", ((epr_uchar*) field->elems)[i]);
                break;
            case e_tid_char:
                fprintf(ostream, "%d", ((char*) field->elems)[i]);
                break;
            case e_tid_ushort:
                fprintf(ostream, "%u", ((epr_ushort*) field->elems)[i]);
                break;
            case e_tid_short:
                fprintf(ostream, "%d", ((short*) field->elems)[i]);
                break;
            case e_tid_uint:
                fprintf(ostream, "%u", ((epr_uint*) field->elems)[i]);
                break;
            case e_tid_int:
...
void epr_print_element(const EPR_SRecord* record, epr_uint field_index, epr_uint element_index, FILE* ostream)
{
    ...
    else if (field->info->data_type_id == e_tid_time)
    {
        ...
        {
            case e_tid_uchar:
                fprintf(ostream, "%u", ((epr_uchar*) field->elems)[element_index]);
                break;
            case e_tid_char:
                fprintf(ostream, "%d", ((char*) field->elems)[element_index]);
                break;
            case e_tid_ushort:
                fprintf(ostream, "%u", ((epr_ushort*) field->elems)[element_index]);
                break;
            case e_tid_short:
                fprintf(ostream, "%d", ((short*) field->elems)[element_index]);
                break;
            case e_tid_uint:
                fprintf(ostream, "%u", ((epr_uint*) field->elems)[element_index]);
                break;
            case e_tid_int:
