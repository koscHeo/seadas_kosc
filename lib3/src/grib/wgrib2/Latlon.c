...
int f_ijlat(ARG2) {
	...
	*local = save;
    ...
    sprintf(inv_out,"(%d,%d),lon=%g,lat=%g,val=%lg",save->ix,save->iy,
...
int f_ilat(ARG1) {
    ...
    sprintf(inv_out,"grid pt %d,lon=%g,lat=%g,val=%lg",i,
...
int f_lon(ARG2) {
    ...
    sprintf(inv_out,"lon=%g,lat=%g,val=%lg",lon[save->iptr],lat[save->iptr],data[save->iptr]);
    return 0;
}
