...
! 
        ...
        elseif (kgds(1).eq.5) then       ! Polar Stereographic Grid
         ...
         elseif (kgds(1).eq.204) then       ! Curivilinear Orthogonal Grid (Used by RTOFS)
           ...
           igdstmpl(18)=0
           igdstmpl(19)=kgds(11)          ! Scanning mode
C        elseif (kgds(1).eq.203) then    !  Rot Lat/Lon grid (Arakawa)
C           idefnum=0
C           igds(1)=0                 ! grid def specfied in template
C           igds(2)=kgds(2)*kgds(3)   ! num of grid points
C           igds(3)=0                 ! octets for further grid definition
C           igds(4)=0                 ! interpretation of optional list
C           igds(5)=1                 ! Grid Definition Template number
C           if ( btest(kgds(6),6) ) then     ! shape of Earth
C             igdstmpl(1)=2
C           else
C             igdstmpl(1)=0
C           endif
...
C           igdstmpl(6)=0
C           igdstmpl(7)=0
C           igdstmpl(8)=kgds(2)      !Ni
C           igdstmpl(9)=kgds(3)      !Nj
C           igdstmpl(10)=0
C           igdstmpl(11)=0
C           igdstmpl(12)=kgds(4)*1000      ! Lat of 1st grid point
C           if ( kgds(5).lt.0 ) then       ! Lon of 1st grid point
C              igdstmpl(13)=(360000+kgds(5))*1000    ! convert W to E
C           else
C              igdstmpl(13)=kgds(5)*1000
C           endif
C           igdstmpl(14)=0                 ! Resolution and Component flags
C           if ( btest(kgds(6),7) ) igdstmpl(14)=48
C           if ( btest(kgds(6),3) ) igdstmpl(14)=igdstmpl(14)+8
!->           igdstmpl(15)=kgds(7)*1000      ! Lat of last grid point
!->           if ( kgds(8).lt.0 ) then       ! Lon of last grid point
!->              igdstmpl(16)=(360000+kgds(8))*1000    ! convert W to E
!->           else
!->              igdstmpl(16)=kgds(8)*1000
!->           endif
C           igdstmpl(17)=kgds(9)*1000      ! Di
C           igdstmpl(18)=kgds(10)*1000     ! Dj
C           igdstmpl(19)=kgds(11)          ! Scanning mode
!->        igdstmpl(20)=
!->        igdstmpl(21)=
!->        igdstmpl(22)=
C           if (kgds(20).ne.255) then         !  irregular grid (eg WAFS)
C               igds(2)=kgds(21)              ! num of grid points
C               !idefnum=kgds(19)
C               if (kgds(2).eq.65535) idefnum=kgds(3)
C               if (kgds(3).eq.65535) idefnum=kgds(2)
C               imax=0
C               do j=1,idefnum
C                  ideflist(j)=kgds(21+j)
C                  if (ideflist(j).gt.imax) imax=ideflist(j)
C               enddo
C               igds(3)=1                 ! octets for further grid definition
C               if (imax.gt.255) igds(3)=2
C               if (imax.gt.65535) igds(3)=3
C               if (imax.gt.16777215) igds(3)=4
C               igds(4)=1                 ! interpretation of optional list
C               igdstmpl(8)=-1
C               igdstmpl(17)=-1
C           endif
        else
