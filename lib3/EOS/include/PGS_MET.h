...
HISTORY:
	...
	31-May-95 ANS  Updates resulting from the code inspection
        ...
        24-Apr-2002 AT Added new flags for opening HDF5 files using
                       PGS_MET_SDstart()
        02-May-2009 At Added new header for ODL to XML converter
...
#define PGSd_MET_LOGICAL_XML 10256 /*XML file writing flag for core metadata*/
...
#ifndef MAX_SPLIT_ELEM
#define MAX_SPLIT_ELEM 10000
#endif
...
typedef struct
{
  ...
  Instruments instrument[MAXINST];
  int instrumentCount;
}
  Platforms;
...
typedef struct
{
  char DataCenterId[128];
  char SdsrvDbID[128];
  char ProductionDateTime[128];
  char InsertTime[128];
  char UpdateTime[128];
  char SizeOfGranule[128];
  char EmptyBrowse[128];
}
ssProperty;
...
PGS_MET_GetSetAttrTD(
       ...
       void *          parmValue);               /* Parameter value buffer
				          	  * to hold the retrieved value
					          */
/* prototypes for functions used for XML metadata implementation */


PGSt_integer 
PGS_MET_ODLToXML(char *theOdlFilename, char *xmlFilename);

PGSt_SMF_status 
PGS_MET_XslProcessor(char *stylesheet, 
		     char *Raw_XMLFile, 
		     char *OUT_XMLFile, 
...
PGS_MET_WriteXML(
		 ...
		 char *hdfAttrName, /*XML hdf attribute name to be attached */
		 PGSt_integer hdfFileId); /*HDF product file id */
...
#ifndef _PGS_MET_ODL2XML_H
#define _PGS_MET_ODL2XML_H
#endif
